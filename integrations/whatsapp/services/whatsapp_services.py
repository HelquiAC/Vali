...
logger = get_logger('whatsapp.services.whatsapp_services')
...
async def service_handle_webhook(request: Request):
    logger.info('Execute fun service_handle_webhook')
...
async def process_webhook_background(message_user: WhatsappPayload):
    logger.info('Execute fun process_webhook_background')
    id_user = await validate_id_user(message_user.entry[0].changes[0].value.messages[0].from_)
    if id_user:
        await send_message_whatsapp_to_conversations_microservice(message_user)      
    else:
        logger.error('Problem validating the id_user')